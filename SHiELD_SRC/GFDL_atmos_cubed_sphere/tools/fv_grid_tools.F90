...
!
!REPLACE kinked with extended grid
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
if ( Atm%flagstruct%duogrid)    then

do n=1,ndims
do j=jsd,jed+1
do i=isd,ied+1
   grid(i,j,n) = Atm%gridstruct%dg%b_pt(n,i,j)
enddo
enddo
enddo

endif
if (.not. Atm%flagstruct%duogrid)             call mpp_update_domains( grid, Atm%domain, position=CORNER)
             if (.not. (Atm%gridstruct%bounded_domain)) then
...
if (.not. Atm%flagstruct%duogrid)                call get_symmetry(dx(is:ie,js:je+1), dy(is:ie+1,js:je), 0, 1, Atm%layout(1), Atm%layout(2), &
                     Atm%domain, Atm%tile_of_mosaic, Atm%gridstruct%npx_g, Atm%bd)
             endif

if (.not. Atm%flagstruct%duogrid)             call mpp_get_boundary( dy, dx, Atm%domain, ebufferx=ebuffer, wbufferx=wbuffer, sbuffery=sbuffer, nbuffery=nbuffer,&
...
if (.not. Atm%flagstruct%duogrid)             call mpp_update_domains( dy, dx, Atm%domain, flags=SCALAR_PAIR,      &
...
if (.not. Atm%flagstruct%duogrid)             call mpp_update_domains( agrid, Atm%domain, position=CENTER, complete=.true. )
             if (.not. (Atm%gridstruct%bounded_domain) .and. (.not. Atm%flagstruct%duogrid) ) then
...
if (.not. Atm%flagstruct%duogrid)       call mpp_update_domains( area,   Atm%domain, complete=.true. )
...
if (.not. Atm%flagstruct%duogrid)       call mpp_update_domains( area_c, Atm%domain, position=CORNER, complete=.true.)
