...
module fv_control_mod
   ...
   use duogrid_mod, only: duogrid_init
...
!-------------------------------------------------------------------------------
   ...
   subroutine fv_control_init(Atm, dt_atmos, this_grid, grids_on_this_pe, p_split)
     ...
     integer :: n, npes, pecounter, i, j, num_family, ntiles_nest_all, num_tile_top
     ...
     call domain_decomp(this_grid,Atm(this_grid)%flagstruct%npx,Atm(this_grid)%flagstruct%npy,Atm(this_grid)%flagstruct%ntiles,&
          ...
          Atm(this_grid)%domain_for_read,Atm(this_grid)%num_contact,Atm(this_grid)%pelist)
     ...
     if (this_grid==1 .and. Atm(this_grid)%flagstruct%duogrid) then
       call duogrid_init(Atm(this_grid))
       Atm(1)%gridstruct%dg%layout = Atm(1)%layout
     endif


     if (mpp_pe() == 0) print*, "DUO-GRID is: ", Atm(this_grid)%flagstruct%duogrid, atm(1)%gridstruct%dg%is_initialized
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!do j= Atm(this_grid)%bd%jsd, Atm(this_grid)%bd%jed+1
!do i= Atm(this_grid)%bd%isd, Atm(this_grid)%bd%ied+1
!write(100+mpp_pe(),*), 'ijdpt1', i, j, Atm(this_grid)%gridstruct%grid_64(i,j,1)
!write(200+mpp_pe(),*), 'ijdpt2', i, j, Atm(this_grid)%gridstruct%grid_64(i,j,2)
!enddo
!enddo
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
