...
! scheme 2.1: perturbation form
  ...
  real, parameter:: b5 = -0.05
      ...
      public :: c_sw, d_sw1, d_sw2, d_sw3, d_sw4, d_sw5, d_sw6, fill_4corners, del6_vt_flux, divergence_corner, divergence_corner_nest
...
! <<< Save the mass fluxes to the "Flux Capacitor" for tracer transport >>>
   ...
   subroutine d_sw2( delp, ptc, pt, w, q_con, &
      ...
      real,intent(OUT) :: dw(bd%is:bd%ie,bd%js:bd%je) !  w damping to be applied in dsw5
      real, intent(OUT),   dimension(bd%isd:bd%ied,  bd%jsd:bd%jed)  ::  ptc
...
! Local:
      ...
      real, dimension(bd%isd:bd%ied,  bd%jsd:bd%jed+1) :: u2
      real, dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed  ) :: v2
      ...
      norm_tgx_d    => gridstruct%norm_tgx_d
      norm_tgy_c    => gridstruct%norm_tgy_c
...
! Compute B grid contra-variant components for KE:
      ...
      if(flagstruct%adv_scheme==1) then
         ...
         call ytp_v(is,ie,js,je,isd,ied,jsd,jed, vb_av, v2, ub, hord_mt, gridstruct%dy, gridstruct%rdy, &
                    npx, npy, flagstruct%grid_type, gridstruct, .false., flagstruct%lim_fac, &
                    flagstruct%adv_scheme)
         do j=js,je+1
            do i=is,ie+1
      ...
      if(flagstruct%adv_scheme==1) then
         ...
         call xtp_u(is,ie,js,je, isd,ied,jsd,jed, ub_av, u2, vb, hord_mt, gridstruct%dx, gridstruct%rdx, &
                    npx, npy, flagstruct%grid_type, gridstruct, .false., flagstruct%lim_fac, &
                    flagstruct%adv_scheme)
         do j=js,je+1
            do i=is,ie+1
...
! Vorticity transport
   ...
   subroutine d_sw6( delp, u,  v, &
      ...
      type(fv_grid_type), intent(IN), target :: gridstruct
      type(fv_flags_type), intent(IN), target :: flagstruct
      real, intent(INOUT),   dimension(bd%is:bd%ie,bd%js:bd%je):: diss_est
! Local:
!---
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! similar to divergence_corner_nest with a fix at the 4corners
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!DEC$ VECTOR ALWAYS
         do i=is,ie+1
            if(adv_scheme==1) then
               ...
               cfl = c(i,j)*rdyc(i)*rnorm_tgy_b(i,j)
               if( c(i,j)>0. ) then
                  !cfl = c(i,j)*rdy(i,j-1)
                  flux(i,j) = v(i,j-1) + (1.-cfl)*(br(i,j-1)-cfl*b0(i,j-1))
               else
...
subroutine departure_cfl_rk2_bgrid_x(gridstruct, flagstruct, bd, ub_old, ub, ub_av, dt)
    ...
    rdxa => gridstruct%rdxa_cs
    rnorm_tgx_b => gridstruct%rnorm_tgx_b
     norm_tgx_b => gridstruct% norm_tgx_b
    ...
    do j = js, je+1
       do i = is, ie+1
          ...
          ub_av(i,j) = ub_av(i,j)*norm_tgx_b(i,j)
       end do
    end do
...
subroutine departure_cfl_rk2_bgrid_y(gridstruct, flagstruct, bd, vb_old, vb, vb_av, dt)
    ...
    rdya => gridstruct%rdya_cs
    rnorm_tgy_b => gridstruct%rnorm_tgy_b
     norm_tgy_b => gridstruct% norm_tgy_b
    ...
    do j = js, je+1
       do i = is, ie+1
          ...
          vb_av(i,j) = vb_av(i,j)*norm_tgy_b(i,j)
       end do
    end do
