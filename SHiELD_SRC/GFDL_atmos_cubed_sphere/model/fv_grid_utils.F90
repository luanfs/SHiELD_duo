...
#include <fms_platform.h>
 ...
 use fv_arrays_mod,   only: fv_atmos_type, fv_grid_type, fv_flags_type, fv_grid_bounds_type, &
                            R_GRID
...
! Higher precision (kind=16) for grid geometrical factors:
 ...
 public symm_grid, cubed_a2d, compute_conversion_matrices, gnomonic_grid_local_coords, metricterm, gnomonic_grid_b2acd,&
        tgvector_norms
...
!     ess => Atm%gridstruct%ess
      ...
      Atm%gridstruct%adv_scheme = Atm%flagstruct%adv_scheme
...
!          if(.not.hydrostatic) gridstruct%w00(i,j) = 2.d0*omega*cos(agrid(i,j,2))
 ...
 subroutine cubed_to_latlon(u, v, ua, va, gridstruct, flagstruct, npx, npy, km, mode, grid_type, domain, bounded_domain, c2l_ord, bd)
 type(fv_grid_bounds_type), intent(IN) :: bd
 ...
 if ( c2l_ord == 2 ) then
      ...
      call c2l_ord4(u, v, ua, va, gridstruct, flagstruct, npx, npy, km, grid_type, domain, bounded_domain, mode, bd)
 endif

 end subroutine cubed_to_latlon


 subroutine c2l_ord4(u, v, ua, va, gridstruct, flagstruct, npx, npy, km, grid_type, domain, bounded_domain, mode, bd)
...
! 2nd order:
  ...
  subroutine compute_conversion_matrices(bd, gridstruct)
     ...
     !---------------------------------------------------------------------
      ...
      type(fv_grid_bounds_type), intent(IN) :: bd
      ! Integer auxs
      integer :: i, j ! 2D grid counters
      integer :: is , js , ie , je
      integer :: isd, jsd, ied, jed

      ! Real aux vars
      real(kind=R_GRID) :: elon(1:3), elat(1:3)
      real(kind=R_GRID) :: ex(1:3), ey(1:3) ! vectors
      ...
      do i = isd, ied+1
         do j = jsd, jed
            ! get latlon tangent vector
            call unit_vect_latlon_ext(cgrid(i,j,1:2), elon, elat)
            if(i<ied+1)then
               call latlon2xyz(cgrid(i,j,1:2), p0)
               call latlon2xyz(agrid(i,j,1:2), px)
               call latlon2xyz( grid(i,j+1,1:2), py)
               ...
               call latlon2xyz(cgrid(i,j,1:2), p0)
               call latlon2xyz(agrid(i-1,j,1:2), px)
               call latlon2xyz( grid(i,j+1,1:2), py)
            ...
            c_contra2l(1,1,i,j) = a11
            c_contra2l(1,2,i,j) = a12
            c_contra2l(2,1,i,j) = a21
            c_contra2l(2,2,i,j) = a22

            ! latlon to contra matrix
            c_l2contra(1,1,i,j) =  a22/det
            c_l2contra(1,2,i,j) = -a12/det
            c_l2contra(2,1,i,j) = -a21/det
            c_l2contra(2,2,i,j) =  a11/det

            ! latlon to covari matrix 
            c_l2covari(1,1,i,j) = a11
            c_l2covari(1,2,i,j) = a21 
            c_l2covari(2,1,i,j) = a12
            c_l2covari(2,2,i,j) = a22
      ...
      do i = isd, ied
         do j = jsd, jed+1
            ! get latlon tangent vector
            call unit_vect_latlon_ext(dgrid(i,j,1:2), elon, elat)
            if(j<jed+1)then
               call latlon2xyz(dgrid(i,j,1:2), p0)
               call latlon2xyz(agrid(i,j,1:2), py)
               call latlon2xyz( grid(i+1,j,1:2), px)
               ...
               call latlon2xyz(dgrid(i,j,1:2), p0)
               call latlon2xyz(agrid(i,j-1,1:2), py)
               call latlon2xyz( grid(i+1,j,1:2), px)
            ...
            ! Contra to latlon matrix
            d_contra2l(1,1,i,j) = a11
            d_contra2l(1,2,i,j) = a12
            d_contra2l(2,1,i,j) = a21
            d_contra2l(2,2,i,j) = a22

            ! latlon to contra matrix
            d_l2contra(1,1,i,j) =  a22/det
            d_l2contra(1,2,i,j) = -a12/det
            d_l2contra(2,1,i,j) = -a21/det
            d_l2contra(2,2,i,j) =  a11/det

            ! latlon to covari matrix
            d_l2covari(1,1,i,j) =  a11
            d_l2covari(1,2,i,j) =  a21
            d_l2covari(2,1,i,j) =  a12
            d_l2covari(2,2,i,j) =  a22
...
! Update:
 ...
 subroutine update2d_dwinds_phys(is, ie, js, je, isd, ied, jsd, jed, dt, u_dt, v_dt, u, v, bd, gridstruct, flagstruct, npx, npy, npz, domain)
...
if (.not. gridstruct%dg%is_initialized) then
  call mpp_update_domains(u_dt, v_dt, domain, gridtype=AGRID_PARAM)
     ...
     call ext_vector(u_dt, v_dt, gridstruct%dg, bd, domain, gridstruct, flagstruct,0,0,0,0)
endif
       call timing_off('COMM_TOTAL')
...
! East Edges:
 ...
 subroutine cubed_a2d( npx, npy, npz, ua, va, u, v, gridstruct, flagstruct, fv_domain, bd )
...
if (.not. gridstruct%dg%is_initialized) then
  ...
  call mpp_update_domains(va, fv_domain, complete=.true.)
   ...
   call ext_scalar(ua, gridstruct%dg, bd, fv_domain, 0,0)
   call ext_scalar(va, gridstruct%dg, bd, fv_domain, 0,0)
   !  call ext_vector(ua, va, gridstruct%dg, bd, fv_domain, gridstruct, flagstruct,0,0,0,0)
endif
    im2 = (npx-1)/2
    jm2 = (npy-1)/2
...
!                                                       ^^^^^^^^^^^^^^^^^^^^^^^^
 ...
 subroutine gnomonic_grid_local_coords(grid_type, dglog, bd, agrid, bgrid, line_a, line_b)
 ! type of grid: must be 0, 1 or 2.
 integer, intent(inout):: grid_type
 logical, intent(in):: dglog
 real(kind=R_GRID), intent(inout), pointer, dimension(:,:,:) :: agrid, bgrid
 real(kind=R_GRID), intent(inout), pointer, dimension(:) :: line_a, line_b
 real(kind=R_GRID) :: a_ll(2), a_xyz(3)
 real(kind=R_GRID) :: b_ll(2), b_xyz(3)
 real(kind=R_GRID) :: y_coord
 type(fv_grid_bounds_type), intent(IN) :: bd
 ...
 ! find panel
 a_ll(1:2) = agrid(is,js,1:2)
 call latlon2xyz(a_ll, a_xyz)
 call find_cube_panel(a_xyz, panel)

 ! compute the cube coordinates of the A grid points
 do i = isd, ied
    a_ll(1:2) = agrid(i,js,1:2)
    call latlon2xyz(a_ll, a_xyz)
    call inverse_gnomonic_map(a_xyz, line_a(i), y_coord, panel, grid_type)
 enddo

 ! compute the cube coordinates of the B grid points
 do i = isd, ied+1
    b_ll(1:2) = bgrid(i,js,1:2)
    call latlon2xyz(b_ll, b_xyz)
    call inverse_gnomonic_map(b_xyz, line_b(i), y_coord, panel, grid_type)
 enddo

 if(.not. dglog) then
   line_b(isd:is-1) = line_b(ie-ng+2:ie+1)
   line_b(ie+2:ied+1) = line_b(is:is+ng-1)
   line_a(isd:is-1) = line_a(ie-ng+1:ie)
   line_a(ie+1:ied) = line_a(is:is+ng-1)
 endif


 end subroutine gnomonic_grid_local_coords
 ...
 subroutine tgvector_norms(grid_type, x, y, norm_tgx, norm_tgy, erad)
 integer, intent(in):: grid_type
 ! type of grid: must be 0 or 2.
 real(kind=R_GRID), intent(in) :: x, y, erad
 real(kind=R_GRID), intent(out) :: norm_tgx, norm_tgy
 real(kind=R_GRID) :: aref, Rref, aref2, Rref2
 real(kind=R_GRID) :: tanx, tany, tanxtany, r
 real(kind=R_GRID) :: cosx, cosy
 real(kind=R_GRID) :: cosx2, cosy2
 real(kind=R_GRID) :: tanx2, tany2
 ...
 r = 1.d0 + Rref2*tanx2 + Rref2*tany2
 norm_tgx = dsqrt(1.d0 + Rref2*tany2)
 norm_tgy = dsqrt(1.d0 + Rref2*tanx2)

 norm_tgx = erad*Rref*norm_tgx/cosx2
 norm_tgy = erad*Rref*norm_tgy/cosy2

 norm_tgx = norm_tgx/r
 norm_tgy = norm_tgy/r
 end subroutine tgvector_norms


 subroutine find_cube_panel(p, panel)
 !---------------------------------------------------------------
 ! find_cube_pabel
 !
 ! given a point on the sphere (cartesian coords), this routine finds the panel
 ! where p is located
 !
 !---------------------------------------------------------------
 real(kind=R_GRID), intent(in) :: p(1:3) ! given point
 ...
 if(abs(p(1))>=abs(p(2)) .and. abs(p(1))>=abs(p(3)) .and. p(1)>0.d0)then
   panel = 1 

 else if(abs(p(2))>=abs(p(1)) .and. abs(p(2))>=abs(p(3)) .and. p(2)>0.d0)then
   panel = 2

 else if(abs(p(1))>=abs(p(2)) .and. abs(p(1))>=abs(p(3)) .and. p(1)<0.d0)then
   panel = 4

 else if(abs(p(2))>=abs(p(1)) .and. abs(p(2))>=abs(p(3)) .and. p(2)<0.d0)then
   panel = 5

 else if(abs(p(3))>=abs(p(1)) .and. abs(p(3))>=abs(p(2)) .and. p(3)>0.d0)then
   panel = 3
 ...
 if(panel == -1)then
   call mpp_error(FATAL, 'error on find_cube_panel: couldnt find the panel.')
 end if

 end subroutine find_cube_panel


 subroutine gnomonic_map(p, x, y, panel, grid_type)
 !---------------------------------------------------------------
 ! compute gnomonic_map given in grid_type
 ! 0-equiedge
 ! 1-equidistant
 ! 2-equiangular
 !---------------------------------------------------------------
 real(kind=R_GRID), intent(out) :: x ! cube coordinates
 real(kind=R_GRID), intent(out) :: y ! cube coordinates
 real(kind=R_GRID), intent(inout) :: p(1:3) ! point on the sphere
 integer, intent(out) :: panel
 integer, intent(out) :: grid_type

 select case(grid_type)
   case(0)
     call equiedge_gnomonic_map(p, x, y, panel)

   case(1)
     call equidistant_gnomonic_map(p, x, y, panel)

   case(2)
     call equiangular_gnomonic_map(p, x, y, panel)

   case default
     call mpp_error(FATAL, 'error on gnomonic_map: invalid grid_type')
  end select
  return
 end subroutine gnomonic_map

 subroutine inverse_gnomonic_map(p, x, y, panel, grid_type)
 !---------------------------------------------------------------
 ! compute inverse gnomonic_map given in grid_type
 ! 0-equiedge
 ! 1-equidistant
 ! 2-equiangular
 !---------------------------------------------------------------
 ...
 select case(grid_type)
   case(0)
     call inverse_equiedge_gnomonic_map(p, x, y, panel)
   case(1)
     call inverse_equidistant_gnomonic_map(p, x, y, panel)
   case(2)
     call inverse_equiangular_gnomonic_map(p, x, y, panel)
   case default
     call mpp_error(FATAL, 'error on inverse_gnomonic_map: invalid grid_type')
  end select
  return
 end subroutine inverse_gnomonic_map
...
!---------------------------------------------------------------------
 ...
 select case(panel)
   ...
   case default
     call mpp_error(FATAL, 'error on equidistant_gnomonic_map: invalid panel')
 end select
 p = p/norm2(p)
 return
 ...
 subroutine equiangular_gnomonic_map(p, x, y, panel)
 !---------------------------------------------------------------
 ...
 !
 !---------------------------------------------------------------
 real(kind=R_GRID), intent(in) :: x ! cube coordinates
 real(kind=R_GRID), intent(in) :: y ! cube coordinates
 real(kind=R_GRID), intent(out) :: p(1:3) ! point on the sphere

 ! panel
 integer, intent(in) :: panel

 ! aux vars
 real(kind=R_GRID) :: tanx, tany

 ! auxiliary variables
 tanx = dtan(x)
 tany = dtan(y)

 ! compute the point using equidistant map
 call equidistant_gnomonic_map(p, tanx, tany, panel)
 return
 end subroutine equiangular_gnomonic_map

 subroutine equiedge_gnomonic_map(p, x, y, panel)
 !---------------------------------------------------------------
 ! this routine computes the of the gnomonic mapping based on the equiedge projection
 ! defined by X. Chen 2021 for each panel
 ! - returns the cube coordinates of the
 ! projected points.
 !
 !---------------------------------------------------------------
 ...
 ! aux vars
 real(kind=R_GRID) :: tanx, tany
 real(kind=R_GRID) :: r, r2, invr,  xor, yor, aref, Rref
 ...
 call equidistant_gnomonic_map(p, tanx, tany, panel)
 return
 end subroutine equiedge_gnomonic_map
 ...
 real(kind=R_GRID), intent(out) :: x ! cube coordinates
 real(kind=R_GRID), intent(out) :: y ! cube coordinates
 real(kind=R_GRID), intent(in) :: p(1:3) ! point on the sphere
 ...
 select case(panel)
   ...
   case(6)
     ...
     y = -p(1)/p(3)
   case default
     call mpp_error(FATAL, 'error on inverse_equidistant_gnomonic_map: invalid panel')
 end select
 return
 ...
 ! aux vars
 real(kind=R_GRID) :: tanx, tany, Rref
 ...
 subroutine gnomonic_grid_b2acd(grid_type, b1, b2, b3, b4, ageo, acube, c1, c2, d1, d2)
 ! this routine gets B grid points (b1, b2, b3, b4)  and its center (ageo, geodesical center computed using cell_center2)
 ! of a cell and compute its A grid point in local cube coordinates (acube). 
 ! it also compute the C grid points (c1, c2) and D grid points (d1, d2)
 !
 ! b3-------d2-------b4
 ! |                 |
 ! |                 |
 ...
 ! |                 |
 ! b1-------d1-------b2
 ! the input points are given latlon coordinates
 ! type of grid: must be 0, 1 or 2.
 integer, intent(inout):: grid_type

 ! lat/lon coordiantes inputs
 real(kind=R_GRID) :: b1(2), b2(2), b3(2), b4(2)
 real(kind=R_GRID), intent(out):: ageo(2), acube(2)
 real(kind=R_GRID), intent(out):: c1(2), c2(2)
 real(kind=R_GRID), intent(out):: d1(2), d2(2)

 ! xyz coordinates points
 real(kind=R_GRID) :: pb1(3), pb2(3), pb3(3), pb4(3) ! B grid
 real(kind=R_GRID) :: pa_geo(3), pa_cube(3) ! A grid
 real(kind=R_GRID) :: pc1(3), pc2(3) ! C grid
 real(kind=R_GRID) :: pd1(3), pd2(3) ! D grid
 
 ! aux vars - cubed sphere coordinates
 real(kind=R_GRID) :: x_b1, y_b1
 real(kind=R_GRID) :: x_b2, y_b2
 real(kind=R_GRID) :: x_b3, y_b3
 real(kind=R_GRID) :: x_b4, y_b4
 real(kind=R_GRID) :: x_c1 , y_c1
 real(kind=R_GRID) :: x_c2 , y_c2
 real(kind=R_GRID) :: x_d1 , y_d1
 real(kind=R_GRID) :: x_d2 , y_d2
 real(kind=R_GRID) :: x_a , y_a

 ! panel finder
 integer :: panel

 ! Convert latlon to cartesian coordinates
 call latlon2xyz(b1, pb1)
 call latlon2xyz(b2, pb2)
 call latlon2xyz(b3, pb3)
 call latlon2xyz(b4, pb4)
 call latlon2xyz(ageo, pa_geo)

 ! now let us find the cube panel where pa_geo is (and so are the B grid points)
 call find_cube_panel(pa_geo, panel)

 ! compute the cube coordinates of the B grid points
 call inverse_gnomonic_map(pb1, x_b1, y_b1, panel, grid_type)
 call inverse_gnomonic_map(pb2, x_b2, y_b2, panel, grid_type)
 call inverse_gnomonic_map(pb3, x_b3, y_b3, panel, grid_type)
 call inverse_gnomonic_map(pb4, x_b4, y_b4, panel, grid_type)

 ! midpoint on cube coordinates
 x_a = (x_b1 + x_b2 + x_b3 + x_b4)*0.25d0
 y_a = (y_b1 + y_b2 + y_b3 + y_b4)*0.25d0

 x_d1 = (x_b1 + x_b3)*0.5d0
 y_d1 = (y_b1 + y_b3)*0.5d0

 x_d2 = (x_b2 + x_b4)*0.5d0
 y_d2 = (y_b2 + y_b4)*0.5d0

 x_c1 = (x_b1 + x_b2)*0.5d0
 y_c1 = (y_b1 + y_b2)*0.5d0

 x_c2 = (x_b3 + x_b4)*0.5d0
 y_c2 = (y_b3 + y_b4)*0.5d0


 ! compute the new midpoint
 call gnomonic_map(pa_cube, x_a, y_a, panel, grid_type)
 call gnomonic_map(pc1, x_c1, y_c1, panel, grid_type)
 call gnomonic_map(pc2, x_c2, y_c2, panel, grid_type)
 call gnomonic_map(pd1, x_d1, y_d1, panel, grid_type)
 call gnomonic_map(pd2, x_d2, y_d2, panel, grid_type)

 call cart_to_latlon(1, pa_cube, acube(1), acube(2))
 call cart_to_latlon(1, pc1, c1(1), c1(2))
 call cart_to_latlon(1, pc2, c2(1), c2(2))
 call cart_to_latlon(1, pd1, d1(1), d1(2))
 call cart_to_latlon(1, pd2, d2(1), d2(2))

 end subroutine gnomonic_grid_b2acd
