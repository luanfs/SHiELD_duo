...
! mass fluxes
       ...
       if (gridstruct%bounded_domain .and. .not. gridstruct%dg%is_initialized) then
     ...
     if ( flagstruct%fv_debug ) then
       ...
       enddo
         ...
         call Lagrangian_to_Eulerian(last_step, consv_te, ps, pe, delp,          &
                     ...
                     ptop, ak, bk, pfull, gridstruct, flagstruct, domain,   &
...
911  call cubed_to_latlon(u, v, ua, va, gridstruct, flagstruct, &
...
if (.not. gridstruct%dg%is_initialized) then
    ...
    call ext_scalar(u2f, gridstruct%dg, bd, domain, 0,0)
endif
                                        call timing_off('COMM_TOTAL')
