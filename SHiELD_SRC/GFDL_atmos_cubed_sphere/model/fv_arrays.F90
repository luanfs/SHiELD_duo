...
!--- MAY NEED TO TEST THIS
  ...
  type fv_grid_bounds_type
   ...
   type duogrid_type
        ...
        ! the location of the grid is follow the location of the uwind
        ! e.g. cpt =  grid pt defined at where u wind in c-grid
        !
        ...
        real(kind=R_GRID), allocatable, dimension(:,:) :: & !--- scalars
            a_sina,  &
            a_cosa,  &
            a_x, a_y, a_kik_x, a_kik_y, & !--- only remap a-grid variables
            a_dx, a_dy, a_da, & !--- min req. for a-grid advection
            rda, rdx, rdy, & !--- min req. for a-grid advection
            a_f, a_kik_f !--- coriolis

        real(kind=R_GRID), allocatable, dimension(:,:,:) :: & !--- vectors
            a_pt, a_kik, b_pt, c_pt, d_pt !--- minimum storage of pt for output purpose

        real(kind=R_GRID), allocatable, dimension(:,:,:,:) :: & !--- matrices
            a_gco, a_gct, &
            a_c2l, a_l2c, & !--- co and rll
            a_kik_gco, a_kik_gct, &
            a_kik_c2l, a_kik_l2c

        ! B-grid
        real(kind=R_GRID), allocatable, dimension(:,:) :: & !--- scalars
        b_x, b_y, b_kik_x, b_kik_y, b_sina, b_cosa, &
        b_dx, b_dy, b_da, b_rda, b_rdx, b_rdy

        real(kind=R_GRID), allocatable, dimension(:,:,:,:) :: & !--- matrices
        b_gco, b_gct, b_c2l, b_l2c

        ! C-grid
        real(kind=R_GRID), allocatable, dimension(:,:) :: & !--- scalars
        c_x, c_y, c_kik_x, c_kik_y, c_sina, c_cosa, &
        c_dx, c_dy, c_da, c_rda, c_rdx, c_rdy

        real(kind=R_GRID), allocatable, dimension(:,:,:,:) :: & !--- matrices
        c_gco, c_gct, c_c2l, c_l2c, &
        c_ct2ort_x, c_ort2ct_x ! could be removed

        ! D-grid
        real(kind=R_GRID), allocatable, dimension(:,:) :: & !--- scalars
        d_x, d_y, d_kik_x, d_kik_y, d_sina, d_cosa, &
        d_dx, d_dy, d_da, d_rda, d_rdx, d_rdy

        real(kind=R_GRID), allocatable, dimension(:,:,:,:) :: & !--- matrices
        d_gco, d_gct, d_c2l, d_l2c, &
        d_ct2ort_x, d_ort2ct_x ! could be removed

        ! extended grid metrics for A 2 C/D transform
        real(kind=R_GRID), allocatable, dimension (:,:,:) :: vlon_ext, vlat_ext
        real(kind=R_GRID), allocatable, dimension (:,:,:,:) :: ew_ext, es_ext

        !real(kind=R_GRID), allocatable :: ss_store(:,:,:,:,:,:)
        ...
        real(kind=R_GRID), allocatable :: xp3(:,:,:,:)
        real(kind=R_GRID), allocatable :: xm3(:,:,:,:)
        ...
        logical :: rmp_s  = .false.
        logical :: rmp_n  = .false.
        logical :: rmp_sw = .false.
        logical :: rmp_se = .false.
        logical :: rmp_nw = .false.
...
!   allocate (  Atm%gridstruct%w00(is_2d-1:ie_2d+1,js_2d-1:je_2d+1) )
    ...
    Atm%gridstruct%bounded_domain = Atm%flagstruct%regional .or. Atm%neststruct%nested .or. Atm%flagstruct%duogrid
