...
module dyn_core_mod
  ...
  use mpp_domains_mod,    only: CGRID_NE, BGRID_SW, BGRID_NE, DGRID_NE, mpp_get_boundary, mpp_update_domains,  &
                                domain2d
  ...
  use sw_core_mod,        only: c_sw, d_sw1, d_sw2,d_sw3,d_sw4, d_sw5, d_sw6
...
! when commenting these 4 local and unused variables
! the code crashes in the middle of a run for a layout (7,9)
! at the flux averaging after d_sw1 (see note below)
! otherwise everything seems good and results are reproducible across any layout so far
! testing for C48npz=5
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    real  fx_delp(bd%is:bd%ie+1, bd%js:bd%je,   npz)
    real  fy_delp(bd%is:bd%ie  , bd%js:bd%je+1, npz)
    real  fx_pt(bd%is:bd%ie+1, bd%js:bd%je,   npz)
    real  fy_pt(bd%is:bd%ie  , bd%js:bd%je+1, npz)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

    real  vortfluxx(bd%is:bd%ie+1, bd%js:bd%je,   npz)
    real  vortfluxy(bd%is:bd%ie  , bd%js:bd%je+1, npz)
    real  allflux_x(bd%is:bd%ie+1, bd%js:bd%je,   npz,4+nq)
    real  allflux_y(bd%is:bd%ie  , bd%js:bd%je+1, npz,4+nq)
    real utt(bd%isd:bd%ied+1  ,bd%jsd:bd%jed,npz)  ! D grid zonal wind (m/s)
    real vtt(bd%isd:bd%ied,bd%jsd:bd%jed+1  ,npz)  ! D grid meridional wind (m/s)
    real wkk(bd%isd:bd%ied,bd%jsd:bd%jed,npz)
    real, dimension(bd%is:bd%ie+1,bd%js:bd%je+1,npz):: ubbtemp,vbbtemp,ubb, vbb 
    real, dimension(bd%isd:bd%ied+1,bd%jsd:bd%jed+1,npz):: kee 
    ...
    real ::  tempfy1(bd%isd:bd%ied+1, bd%jsd:bd%jed+1, npz)
    real :: dw(bd%is:bd%ie  ,bd%js:bd%je,npz )  ! w damping computed in dsw2 applied in dsw5
...
! ----   For external mode:
    ...
    real heat_s(bd%is:bd%ie,bd%js:bd%je, npz)
...
!-------------------------------------
    ...
    integer :: nord_k(npz+1), nord_w(npz+1), nord_t
    integer :: ms
!---------------------------------------
    integer :: i,j,k, it, iq, n_con, nf_ke
    integer :: iep1, jep1
    real    :: beta, beta_d, d_con_k(npz+1), damp_w(npz+1), damp_t, kgb, cv_air
    real    :: dt, dt2, rdt
    real    :: d2_divg(npz+1) ! varies with k in the sponge layers
...
if (.not. duogrid ) then
    ...
    if ( it==1 ) then
                                       call timing_on('COMM_TOTAL')
          if (.not. duogrid ) call complete_group_halo_update(i_pack(1), domain)
...
if (.not. duogrid ) then
      ...
      if ( hydrostatic ) then
           call geopk(ptop, pe, peln, delpc, pkc, gz, phis, ptc, q_con, pkz, npz, akap, .true., &
                      gridstruct%bounded_domain, gridstruct%dg%is_initialized, .false., npx, npy, flagstruct%a2b_ord, bd)
      else
#ifndef SW_DYNAMICS
           if ( it == 1 ) then
                                      ...
                                      call timing_on('COMM_TOTAL')
if (.not. duogrid) then
              call complete_group_halo_update(i_pack(5), domain) !!! gz
else
              call ext_scalar(gz, gridstruct%dg, bd, domain, 0,0) !OK
endif
...
if (.not. duogrid ) then
    ...
    if (flagstruct%inline_q .and. nq>0) call ext_scalar(q, gridstruct%dg, bd, domain, 0,0)
    ...
    if (test_case > 1) then
#endif
                        if (.not. duogrid .and. flagstruct%nord > 0) call complete_group_halo_update(i_pack(3), domain)
...
!      d2_divg = min(0.20, flagstruct%d2_bg*(1.-3.*tanh(0.1*log(pfull(k)/pfull(npz)))))
       d2_divg(k) = min(0.20, flagstruct%d2_bg)
       ...
       damp_w(k) = damp_vt(k)
       damp_t = damp_vt(k)
       d_con_k(k) = flagstruct%d_con

       if ( npz==1 .or. flagstruct%n_sponge<0 ) then
           d2_divg(k) = flagstruct%d2_bg
       else
...
! Vertical velocity:
                   nord_w(k)=0; damp_w(k) = d2_divg(k)
                   if ( flagstruct%do_vort_damp ) then
! damping on delp and vorticity:
                        nord_v(k)=0;
#ifndef HIWPP
                        damp_vt(k) = 0.5*d2_divg(k)
#endif
                   endif
                   d_con_k(k) = 0.
              elseif ( k==2 .and. flagstruct%d2_bg_k2>0.01 ) then
                   nord_k(k)=0; d2_divg(k) = max(flagstruct%d2_bg, flagstruct%d2_bg_k2)
                   nord_w(k)=0; damp_w(k) = d2_divg(k)
                   if ( flagstruct%do_vort_damp ) then
                        nord_v(k)=0;
#ifndef HIWPP
              ...
              elseif ( k==3 .and. flagstruct%d2_bg_k2>0.05 ) then
                   nord_k(k)=0;  d2_divg(k) = max(flagstruct%d2_bg, 0.2*flagstruct%d2_bg_k2)
                   nord_w(k)=0;  damp_w(k) = d2_divg(k)
                   d_con_k (k)= 0.
              endif
       endif
...
!$OMP parallel do default(none) shared(npz,iq,is,ie,js,je,allflux_x, allflux_y,fxx_delp, fyy_delp)
do k=1,npz !! check why K is needed here but not in the next calls
    do j=js,je
      do i=is,ie+1
        fxx_delp(i,j,k)=allflux_x(i,j,k,iq)
      enddo
    enddo
    do j=js,je+1
      do i=is,ie
        fyy_delp(i,j,k)=allflux_y(i,j,k,iq)
      enddo
    enddo
enddo
...
do k=1,npz
    ...
    do j=js,je
      do i=is,ie+1
        allflux_x(i,j,k,iq)=fxx_delp(i,j,k)
      enddo
    enddo
    do j=js,je+1
      do i=is,ie
        allflux_y(i,j,k,iq)=fyy_delp(i,j,k)
      enddo
    enddo
enddo
  ! endif ! if iq
...
!! FOR DSW3 FLUXES
!!!!!!!!!!!!!!!!!!!!!
!$OMP parallel do default(none) shared(npz,is,ie,js,je, tempfx1, tempfy1, ubb, vbbtemp)
  do k=1,npz
    do j=js,je+1
      do i=is,ie+1
        tempfx1(i,j,k)=ubb(i,j,k)
        tempfy1(i,j,k)=vbbtemp(i,j,k)
      enddo
    enddo
  enddo

                                    call timing_on('FLUX_AV_get_boundary')
    call mpp_get_boundary(tempfx1, tempfy1, domain, &
                wbufferx=wbuffer, ebufferx=ebuffer,  &
                sbuffery=sbuffer, nbuffery=nbuffer, gridtype=BGRID_NE )
                                    call timing_off('FLUX_AV_get_boundary')

!$OMP parallel do default(none) shared(npz,is,ie,js,je, tempfx1, tempfy1, sbuffer, nbuffer, ebuffer, &
!$OMP                                  wbuffer, ubb, vbbtemp)
  do k=1,npz
    do i=is,ie+1
      tempfy1(i,js,  k) = 0.5*(tempfy1(i,js,  k)+sbuffer(i-is+1,k))
      tempfy1(i,je+1,k) = 0.5*(tempfy1(i,je+1,k)+nbuffer(i-is+1,k))
    enddo
    do j=js,je+1
      tempfx1(is  ,j,k) = 0.5*(tempfx1(is  ,j,k)+wbuffer(j-js+1,k))
      tempfx1(ie+1,j,k) = 0.5*(tempfx1(ie+1,j,k)+ebuffer(j-js+1,k))
    enddo

    do j=js,je+1
      do i=is,ie+1
        ubb(i,j,k)=tempfx1(i,j,k)
        vbbtemp(i,j,k)=tempfy1(i,j,k)
      enddo
    enddo
  enddo
...
!  do j=js,je+1
!   do i=is,ie+1
!      kee(i,j,k) = (ubbtemp(i,j,k)*vbbtemp(i,j,k))
!      kee(i,j,k) = 0.5*(kee(i,j,k) + ubb(i,j,k)*vbb(i,j,k))
!   enddo
!  enddo
!enddo
!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!! NOT FOR TESTING, ke is computed here
!!!!!!!!!!!!!!!!!!!!!! NOT FOR TESTING, bring it inside dswx 
  do j=js,je+1
   do i=is,ie+1
      kee(i,j,k) = (ubbtemp(i,j,k)*vbbtemp(i,j,k))
      kee(i,j,k) = 0.5*(kee(i,j,k) + ubb(i,j,k)*vbb(i,j,k))
...
!!!!!!!!!!!!!!!!!!!!!!
!
       call d_sw2( delp(isd,jsd,k), ptc(isd,jsd,k),   pt(isd,jsd,k),      &
                  w(isd:,jsd:,k),       &
#ifdef USE_COND
                  q_con(isd:,jsd:,k),  &
#else
                  q_con(isd:,jsd:,1),  &
#endif
                  kgb, heat_s(is,js,k), diss_e, nq,  q,  k,  npz, flagstruct%inline_q,  dt,  &
                  nord_w(k), damp_w(k), hydrostatic, gridstruct, flagstruct, bd, &
                   dw(is,js,k),allflux_x,allflux_y)

!apply KE 
       call d_sw4(u(isd,jsd,k),    v(isd,jsd,k),  dt,  &
                  gridstruct, flagstruct, bd, & 
                  utt(isd,jsd,k),vtt(isd,jsd,k), &
                  kee(isd,jsd,k))

       call d_sw5(vt(isd,jsd,k), delp(isd,jsd,k), ptc(isd,jsd,k),       &
                  ...
                  q_con(isd:,jsd:,1),  z_rat(isd,jsd),  &
                    ...
                    dt,  &
                   hord_v,    &
                  nord_k(k), flagstruct%dddmp, d2_divg(k), flagstruct%d4_bg,  &
...
!apply vort 
       call d_sw6( delp(isd,jsd,k),       &
                  u(isd,jsd,k),    v(isd,jsd,k), &
                   heat_s(is,js,k), diss_e, nord_v(k),  &
                  damp_vt(k), d_con_k(k), gridstruct, flagstruct, bd, &
                  utt(isd,jsd,k),vtt(isd,jsd,k),ubb(is,js,k),vbb(is,js,k),kee(isd,jsd,k),wkk(isd,jsd,k) ,vortfluxx(is,js,k), & 
                  vortfluxy(is,js,k) )
                                                             call timing_on('COMM_TOTAL')
...
! Average horizontal "convergence" to cell center
    ...
    if( flagstruct%fill_dp ) call mix_dp(hydrostatic, w, delp, pt, npz, ak, bk, .false., flagstruct%fv_debug, bd, gridstruct)
                                                             ...
                                                             call timing_on('COMM_TOTAL')
    if (.not. duogrid) call start_group_halo_update(i_pack(1), delp, domain, complete=.false.)
    !call start_group_halo_update(i_pack(1), delp, domain, complete=.false.)
    if (.not. duogrid) call start_group_halo_update(i_pack(1), pt,   domain, complete=.true.)
...
if (.not. duogrid ) then
    ...
    if ( flagstruct%d_ext > 0. ) then
          d2_divg_ct = flagstruct%d_ext * gridstruct%da_min_c
!$OMP parallel do default(none) shared(is,iep1,js,jep1,npz,wk,ptc,divg2,vt,d2_divg_ct)
          do j=js,jep1
              do i=is,iep1
                    wk(i,j) = ptc(i,j,1)
              ...
              enddo
              do i=is,iep1
                 divg2(i,j) = d2_divg_ct*divg2(i,j)/wk(i,j)
              enddo
          enddo
    else
        divg2(:,:) = 0.
...
if (.not. duogrid) then
   ...
   call ext_scalar(q_con, gridstruct%dg, bd, domain, 0, 0)
endif
#endif
                                       call timing_off('COMM_TOTAL')
    if ( flagstruct%fv_debug ) then
    ...
    if (flagstruct%regional) then
     ...
     if ( hydrostatic ) then
          call geopk(ptop, pe, peln, delp, pkc, gz, phis, pt, q_con, pkz, npz, akap, .false., &
                     gridstruct%bounded_domain, flagstruct%duogrid, .true., npx, npy, flagstruct%a2b_ord, bd)
       else
#ifndef SW_DYNAMICS
...
if (.not. duogrid ) then
        ...
        if (.not. duogrid) then
                call complete_group_halo_update(i_pack(4), domain)
                  ...
                  call ext_scalar(zh, gridstruct%dg, bd, domain, 0,0) ! OK
                  call ext_scalar(pkc, gridstruct%dg, bd, domain, 0,0) ! if not square decomp
        endif
        call timing_off('COMM_TOTAL')
!$OMP parallel do default(none) shared(is,ie,js,je,npz,gz,zh,grav)
        ...
        if ( gridstruct%square_domain ) then
           if (.not. duogrid) then
                     ...
                     call ext_scalar(pkc, gridstruct%dg, bd, domain, 0,0) !OK
           endif
        endif
           call timing_off('COMM_TOTAL')
...
if(.not. duogrid) then
    ...
    if( it==n_split .and. gridstruct%grid_type<4) then
...
integer :: is,  ie,  js,  je
      ...
      js  = bd%js
      je  = bd%je
!why this omp crashes if duo on?
